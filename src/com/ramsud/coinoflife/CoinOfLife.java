...
/* @pjs font="data/Clock.ttf, data/Button.ttf"; crisp=true; */ 
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		AdView adView = new AdView(this, AdSize.SMART_BANNER, "ca-app-pub-2072064411205042/1405528013");  
  ...
  public void drawit(int a_width, int c_width, int a_height, int c_height) {
    ...
    for (int i = 0; i < alive.length; i++) {
      for (int j = 2; j < alive[i].length; j++) {
        ...
        if (gem_positions[i][j]) {
          imageMode(CORNER);
          image(G_GEM_IMAGE, i * c_width, j * c_height, (int)(c_width * 1.1f), (int)(c_height * 1.1f));
        }
        if (diamond_positions[i][j]) {
          imageMode(CORNER);
          image(G_DIAMOND_IMAGE, i * c_width, j * c_height, (int)(c_width * 1.1f), (int)(c_height * 1.1f));
        }
        if (rock_positions[i][j]) {
          imageMode(CORNER);
          image(G_ROCK_IMAGE, i * c_width, j * c_height, (int)(c_width * 1.1f), (int)(c_height * 1.1f));
        }
        if (hit_positions[i][j]) {
          imageMode(CENTER);
          image(G_HIT_IMAGE, (int)((i+0.5f) * c_width), (int)((j+0.5f) * c_height), (int)(c_width * 1.8f), (int)(c_height * 1.8f));
        }
      }
    }
...
public class GlobalMenu {
  ...
  public GlobalMenu(int a_width, int c_width, int a_height, int c_height) {
    ...
    int gem_scorer_y = coin_scorer_y + (int)(1.5f * c_height);
    int diamond_scorer_y = gem_scorer_y + (int)(1.5f * c_height);
    int rock_scorer_y = diamond_scorer_y + (int)(1.5f * c_height);
    if (n_vertical <= 10) {
    ...
    continue_button = new CoinButton((int)((width - button_width) / 2), height - 2 * c_height, button_width, button_height, "Next Level");
...
public class StoreDrawer {
  ...
  public StoreDrawer(int a_width, int c_width, int a_height, int c_height) {
    ...
    int button_height = c_height;
    int left_x = 300;
    int right_x = left_x + (int)(2.5f * button_width);
    int button_top = 100;
  ...
  public void drawit(int a_width, int c_width, int a_height, int c_height) {
    ...
    int left_text_x = 170;
    int right_image_x = left_x + (int)(1.5f * button_width);
    int right_text_x = left_x + (int)(1.8f * button_width);
  ...
  // Getter
  public CoinButton getBackButton() {
    return back_button;
  }
